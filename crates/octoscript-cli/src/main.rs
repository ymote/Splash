...
fn profile_output() -> JsonValue {
    ...
    json!({
        ...
        "effect_free_commands": {
            "profile": "octoscript profile",
            "format": "octoscript format [--check] <file>",
            "check": "octoscript check <file>",
            "outline": "octoscript outline <file>",
            "tool_calls": "octoscript tool-calls [--allow-json-add] <file>",
            "workflow_review": "octoscript workflow-review [--allow-json-add] <draft.json>",
            "workflow_schema": "octoscript workflow-schema",
        },
        "tool_api": {
            "import": "use mod.tool",
            "await_method": ".await()",
        ...
        .map(|(step, workflow_step)| {
            let mut direct_module_calls = if step.syntax.valid {
                capability_module_call_hint_report(
                    runtime,
                    &format!("workflow-step-{}.octoscript", step.step_id),
...
fn validate_evaluation_source(source: &str) -> Result<(), String> {
    let report = check_syntax_named("inline.octoscript", source, ExecutionLimits::default())
        .map_err(|error| error.to_string())?;
    if report.valid {
        return Ok(());
    }
    ...
    Err(format!("canonical Octoscript preflight failed: {detail}"))
}
...
fn parse_workflow_grant(value: &str) -> Result<CliWorkflowGrant, String> {
    ...
    if !is_cli_grant_identifier(step_id, MAX_WORKFLOW_STEP_ID_BYTES)
        || !is_cli_grant_identifier(tool, octoscript_capabilities::MAX_TOOL_NAME_BYTES)
    {
...
fn parse_args(args: Vec<String>) -> Result<CliOptions, String> {
    ...
    if format_check && positional.first().is_none_or(|command| command != "format") {
        return Err("--check is only valid with octoscript format".to_owned());
    }
    ...
    {
        return Err("--grant is only valid with octoscript workflow-run".to_owned());
    }
    if workflow_input_path.is_some()
        && positional
            ...
            .is_none_or(|command| command != "workflow-run")
    {
        return Err("--input is only valid with octoscript workflow-run".to_owned());
    }
    ...
    match positional.as_slice() {
        ...
        _ => Err(
            "usage: octoscript profile | octoscript workflow-schema | octoscript check <file> | octoscript outline <file> | octoscript tool-calls [--allow-json-add] <file> | octoscript workflow-review [--allow-json-add] <draft.json> | octoscript workflow-run [--allow-echo] [--allow-json-add] [--input input.json] [--grant step-id:tool-name:max-calls] <draft.json> | octoscript format [--check] <file> | octoscript eval [--allow-echo] [--allow-json-add] '<source>' | octoscript run [--allow-echo] [--allow-json-add] <file> | octoscript catalog [--allow-echo] [--allow-json-add] | octoscript module-catalog [--allow-json-add]".to_owned(),
        ),
    }
}
...
mod tests {
    use super::*;
    use octoscript_workflow::WorkflowStep;
    ...
    fn profile_output_matches_the_canonical_runtime_contract() {
        ...
        assert_eq!(output["language"], json!("Octoscript"));
        ...
        assert_eq!(
            ...
            json!("octoscript profile")
        );
        assert_eq!(
            output["effect_free_commands"]["workflow_schema"],
            json!("octoscript workflow-schema")
        );
    ...
    fn workflow_schema_output_remains_data_only() {
        ...
        assert_eq!(output["x-octoscript"]["authority"], json!("none"));
        assert_eq!(
    ...
    fn check_rejects_makepad_compatibility_syntax() {
        let error = run_options(CliOptions {
            command: CliCommand::Check {
                file: "generated.octoscript".to_owned(),
    ...
    fn eval_rejects_makepad_compatibility_syntax() {
        ...
        assert!(error.starts_with("canonical Octoscript preflight failed:"));
    ...
    fn parses_a_check_invocation() {
        let path = format!(
            "{}/../octoscript-core/tests/fixtures/workflow_language.octoscript",
    ...
    fn parses_a_workflow_run_with_explicit_grants_only() {
        ...
        assert_eq!(
            ...
            .unwrap_err(),
            "--grant is only valid with octoscript workflow-run"
        );
        assert_eq!(
            parse_args(vec![
                "workflow-run".to_owned(),
    ...
    fn parses_a_workflow_run_dataflow_input_only_for_workflow_execution() {
        ...
        assert_eq!(
            ...
            .unwrap_err(),
            "--input is only valid with octoscript workflow-run"
        );
        assert_eq!(
            parse_args(vec![
                "workflow-run".to_owned(),
    ...
    fn outline_keeps_diagnostics_and_rejects_invalid_source() {
        let (output, valid) = outline_output("generated.octoscript", "var value = 1")
    ...
    fn tool_call_outline_resolves_a_configured_direct_module_to_its_target_tool() {
        ...
        let (output, valid) =
            tool_calls_output_with_modules("generated.octoscript", source, Some(&runtime)).unwrap();
    ...
    fn tool_call_outline_marks_omitted_hints_as_truncated() {
        ...
        let (output, valid) = tool_calls_output("generated.octoscript", &source).unwrap();

        assert!(valid);
        assert_eq!(
            output["tool_calls"].as_array().map(Vec::len),
            Some(octoscript_core::MAX_TOOL_CALL_HINTS)
        );
    ...
    fn tool_call_outline_keeps_diagnostics_and_rejects_invalid_source() {
        let (output, valid) = tool_calls_output("generated.octoscript", "var value = 1")
            .expect("invalid source still has structured output");
    ...
    fn workflow_review_bounds_resolved_direct_module_hints_across_steps() {
        ...
        for _ in 0..octoscript_core::MAX_IMPORTED_MODULE_CALL_HINTS {
        ...
        for step in &steps[..4] {
            assert_eq!(
                ...
                Some(octoscript_core::MAX_IMPORTED_MODULE_CALL_HINTS)
            );
    ...
    fn parses_a_format_check_invocation() {
        ...
        assert_eq!(
            ...
            .unwrap_err(),
            "--check is only valid with octoscript format"
        );
    }
