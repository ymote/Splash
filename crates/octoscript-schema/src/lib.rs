...
//! A bounded executable JSON-schema subset for Octoscript capability contracts.
//!
    ...
    let additional_properties = match object.get("additionalProperties") {
        ...
        Some(_) => {
            return Err(invalid_keyword(
                ...
                "must be a boolean in the Octoscript subset",
            ))
        }
        None => true,
    };
...
mod tests {
    ...
    fn validates_nested_object_array_and_scalar_constraints() {
        ...
        let missing = schema
            .validate(&json!({"title": "Ship Octoscript"}))
            .unwrap_err();
