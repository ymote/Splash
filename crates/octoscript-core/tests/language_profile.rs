...
fn workflow_language_fixture_is_compatible_with_the_runtime_profile() {
    ...
    let report = runtime
        .eval(include_str!("fixtures/workflow_language.octoscript"))
        .unwrap();
...
fn grammar_v0_1_fixture_is_accepted_without_capabilities_or_execution() {
    let report = check_syntax(include_str!("fixtures/grammar_v0_1.octoscript")).unwrap();
...
fn grammar_v0_2_fixture_is_accepted_without_capabilities_or_execution() {
    let report = check_syntax(include_str!("fixtures/grammar_v0_2.octoscript")).unwrap();
...
fn canonical_construct_fixture_is_accepted_without_execution() {
    let report = check_syntax(include_str!("fixtures/canonical_constructs.octoscript")).unwrap();
...
fn structured_dataflow_fixture_runs_with_only_standard_modules() {
    let mut runtime = Runtime::default();
    let report = runtime
        .eval(include_str!("fixtures/structured_dataflow.octoscript"))
        .unwrap();
...
fn shipped_workflow_examples_follow_the_canonical_profile() {
    for (name, source) in [
        (
            "deferred_tool_workflow.octoscript",
            include_str!("../../../examples/deferred_tool_workflow.octoscript"),
        ),
        (
            "json_tool_workflow.octoscript",
            include_str!("../../../examples/json_tool_workflow.octoscript"),
        ),
        (
            "tool_workflow.octoscript",
            include_str!("../../../examples/tool_workflow.octoscript"),
        ),
    ] {
