//! Canonical Octoscript v0.2 source-profile validation.
//!
...
//! Octoscript, without producing bytecode or evaluating source.
...
/// Octoscript gives newlines statement-boundary meaning, so a validated source is
...
impl<'source> ProfileLexer<'source> {
            ...
            match character {
                ...
                character => {
                    ...
                    self.report(
                        ...
                        column,
                        format!(
                            "unsupported character `{character}` in the canonical Octoscript profile"
                        ),
                    );
                    self.advance();
                }
    ...
    fn report_token_limit_at(&mut self, line: usize, column: usize) {
        ...
        self.report(
            ...
            column,
            format!(
                "canonical Octoscript token count exceeds the maximum of {}",
                self.max_tokens
            ),
        );
        ...
        match escape {
            ...
            _ => {
                ...
                self.report(
                    line,
                    column,
                    "unsupported string escape in the canonical Octoscript profile",
                );
                self.advance();
            }
        }
    ...
    fn scan_operator(&mut self) {
        ...
        if operator == "~" {
            self.report(
                line,
                column,
                "direct logging (`~`) is not part of the canonical Octoscript profile",
            );
            ...
            self.report(
                line,
                column,
                format!("operator `{operator}` is not part of the canonical Octoscript profile"),
            );
        }
    }
...
impl CanonicalParser {
    ...
    fn enter_nesting(&mut self) -> bool {
        if self.nesting >= self.maximum_nesting {
            self.report_current(format!(
                "canonical Octoscript nesting exceeds the maximum of {}",
