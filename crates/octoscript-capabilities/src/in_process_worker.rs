...
use octoscript_worker::{WorkerJournalStore, WorkerSession, WorkerSessionError};
...
impl<S> InProcessAuthenticatedWorkerTransport<S> {
    ...
    /// it to Octoscript source or untrusted logs.
...
/// `ProtocolWorkerClient` maps every transport error to one generic Octoscript
...
mod tests {
    ...
    impl WorkerAdapter for AddAdapter {
        ...
        fn invoke(
            ...
            _grant: &octoscript_protocol::CapabilityGrant,
