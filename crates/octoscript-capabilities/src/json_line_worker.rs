...
pub use octoscript_protocol::MAX_WIRE_FRAME_BYTES;
...
mod tests {
    ...
    fn one_shot_durable_transport_requires_a_matching_host_session() {
        ...
        let worker_authenticator = SessionAuthenticator::new(
            "worker-1",
            SessionKey::from_bytes([13; octoscript_protocol::AUTH_TAG_BYTES]).unwrap(),
            SessionRole::Worker,
        )
        .unwrap();
        assert!(matches!(
        ...
        let host_authenticator = SessionAuthenticator::new(
            "other-worker",
            SessionKey::from_bytes([14; octoscript_protocol::AUTH_TAG_BYTES]).unwrap(),
            SessionRole::Host,
        )
        .unwrap();
        assert!(matches!(
    ...
    fn sends_a_json_line_and_receives_a_bounded_frame() {
        let key = SessionKey::from_bytes([1; octoscript_protocol::AUTH_TAG_BYTES]).unwrap();
        let mut host =
            SessionAuthenticator::new("worker-1", key.clone(), SessionRole::Host).unwrap();
        let mut worker = SessionAuthenticator::new("worker-1", key, SessionRole::Worker).unwrap();
        let request = host
        ...
        let response = worker
            .seal(WorkerMessage::CloseSession {
                protocol_version: octoscript_protocol::PROTOCOL_VERSION,
    ...
    fn poisons_the_channel_after_a_write_failure() {
        let key = SessionKey::from_bytes([6; octoscript_protocol::AUTH_TAG_BYTES]).unwrap();
    ...
    fn dispatches_an_authenticated_invocation_over_json_lines() {
        ...
        let key = SessionKey::from_bytes([2; octoscript_protocol::AUTH_TAG_BYTES]).unwrap();
        let mut host =
            SessionAuthenticator::new("worker-1", key.clone(), SessionRole::Host).unwrap();
        let mut expected_host =
            SessionAuthenticator::new("worker-1", key.clone(), SessionRole::Host).unwrap();
    ...
    fn dispatches_one_authenticated_durable_operation() {
        let manifest = durable_manifest();
        let key = SessionKey::from_bytes([8; octoscript_protocol::AUTH_TAG_BYTES]).unwrap();
    ...
    fn reconciles_one_authenticated_durable_operation() {
        let manifest = durable_manifest();
        let key = SessionKey::from_bytes([9; octoscript_protocol::AUTH_TAG_BYTES]).unwrap();
        let (host, mut expected_host, mut worker) = opened_operation_session(&manifest, key);
    ...
    fn dispatches_one_authenticated_compensation() {
        ...
        .unwrap();
        let key = SessionKey::from_bytes([10; octoscript_protocol::AUTH_TAG_BYTES]).unwrap();
        let (host, mut expected_host, mut worker) = opened_operation_session(&manifest, key);
    ...
    fn poisons_a_durable_operation_transport_after_an_authenticated_wrong_response() {
        let manifest = durable_manifest();
        let key = SessionKey::from_bytes([11; octoscript_protocol::AUTH_TAG_BYTES]).unwrap();
    ...
    fn poisons_a_durable_operation_transport_after_a_mismatched_result() {
        let manifest = durable_manifest();
        let key = SessionKey::from_bytes([12; octoscript_protocol::AUTH_TAG_BYTES]).unwrap();
        let (host, mut expected_host, mut worker) = opened_operation_session(&manifest, key);
        let request = OperationDispatchRequest::new(
            "worker-1",
            "dispatch-1",
    ...
    fn poisons_the_transport_after_an_invalid_authenticated_response() {
        let manifest = manifest();
        let key = SessionKey::from_bytes([3; octoscript_protocol::AUTH_TAG_BYTES]).unwrap();
        let mut host =
            SessionAuthenticator::new("worker-1", key.clone(), SessionRole::Host).unwrap();
        let mut expected_host =
            SessionAuthenticator::new("worker-1", key.clone(), SessionRole::Host).unwrap();
        ...
        let mut wrong_worker = SessionAuthenticator::new(
            "worker-1",
            SessionKey::from_bytes([4; octoscript_protocol::AUTH_TAG_BYTES]).unwrap(),
            SessionRole::Worker,
        )
        .unwrap();
    ...
    fn rejects_a_non_host_authenticator() {
        let authenticator = SessionAuthenticator::new(
            "worker-1",
            SessionKey::from_bytes([5; octoscript_protocol::AUTH_TAG_BYTES]).unwrap(),
            SessionRole::Worker,
        )
        .unwrap();
