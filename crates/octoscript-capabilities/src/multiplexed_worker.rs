...
impl MultiplexedAuthenticatedWorkerTransport {
        ...
        W: Write + Send + 'static,
    {
        ...
        let join = thread::Builder::new()
            .name("octoscript-host-worker-multiplexer".to_owned())
            .spawn(move || {
                run_coordinator(
                    authorizer,
                    sealer,
...
/// Binding between one claimed Octoscript external tool and one authenticated
...
pub enum ExternalToolWorkerPoll {
    ...
    CancellationTooLate,
    CancellationUnsupported,
    Completed(Option<octoscript_core::Evaluation>),
    Cancelled(Option<octoscript_core::Evaluation>),
}
...
/// Octoscript runtime. A process deadline or force-stop always leaves that runtime
...
{
    thread::Builder::new()
        .name("octoscript-host-worker-frame-reader".to_owned())
        .spawn(move || {
