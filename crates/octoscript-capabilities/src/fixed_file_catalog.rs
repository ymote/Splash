...
//! identifier. Octoscript receives only that identifier through a registered
...
/// errors so paths and operating-system details are never returned to Octoscript.
#[derive(Debug)]
...
impl FixedFileCatalog {
    ...
    ///
    /// The path is never retained or exposed to Octoscript. Symlinks are resolved
    ...
    impl TestFile {
        fn new(label: &str, bytes: &[u8]) -> Self {
            ...
            let path = std::env::temp_dir().join(format!(
                "octoscript-fixed-file-catalog-{label}-{}-{sequence}",
                std::process::id()
            ));
