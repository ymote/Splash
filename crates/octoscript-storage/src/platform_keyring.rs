...
///
/// The service and account are opaque host configuration, never Octoscript data.
...
mod tests {
    ...
    fn entry_rejects_empty_or_noncanonical_credential_identifiers() {
        ...
        assert_eq!(
            PlatformKeyringEntry::new(key_id, "com.ymote.octoscript", "Host User").unwrap_err(),
    ...
    fn configuration_rejects_duplicate_ids_and_credential_locations() {
        let active = entry("storage-v2", "com.ymote.octoscript", "active");
        let duplicate_id = entry("storage-v2", "com.ymote.octoscript", "previous");
        assert_eq!(
        ...
        let duplicate_locator = entry("storage-v1", "com.ymote.octoscript", "active");
        assert_eq!(
    ...
    fn binary_credential_material_must_have_the_storage_key_length() {
        let entry = entry("storage-v1", "com.ymote.octoscript", "active");
    ...
    fn unsupported_targets_do_not_use_a_mock_credential_store() {
        let configuration = PlatformKeyringKeyring::new(
            entry("storage-v1", "com.ymote.octoscript", "active"),
            Vec::new(),
        )
        .unwrap();
