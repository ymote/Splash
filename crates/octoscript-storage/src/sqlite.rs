...
const SCHEMA: &str = "
CREATE TABLE IF NOT EXISTS octoscript_storage_versions (
...
impl<A> AnchoredSqliteStore<A> {
    ...
    /// settings and initializes the Octoscript schema.
    ///
    ...
    /// Configures an existing SQLite connection for anchored Octoscript storage.
    ///
    ...
    A: RollbackAnchor,
{
    ...
    fn record_at_anchor(
        ...
        let record_length: i64 = self
            .connection
            .query_row(
                "SELECT length(CAST(record AS BLOB)) FROM octoscript_storage_versions
        ...
        let bytes = self
            .connection
            .query_row(
                "SELECT CAST(record AS BLOB) FROM octoscript_storage_versions
                 WHERE namespace = ?1 AND name = ?2 AND revision = ?3 AND commitment = ?4
    ...
    fn persist_candidate(
        ...
        let existing = transaction
            .query_row(
                "SELECT record FROM octoscript_storage_versions
                 WHERE namespace = ?1 AND name = ?2 AND revision = ?3 AND commitment = ?4",
                params![
                    key.namespace(),
                    key.name(),
        ...
        let pending: i64 = transaction
            .query_row(
                "SELECT COUNT(*) FROM octoscript_storage_versions
        ...
        transaction
            .execute(
                "INSERT INTO octoscript_storage_versions(namespace, name, revision, commitment, record)
                 VALUES(?1, ?2, ?3, ?4, ?5)",
                params![
                    key.namespace(),
                    key.name(),
...
fn prune_history(
    ...
    transaction.execute(
        "DELETE FROM octoscript_storage_versions
...
fn discard_unanchored_candidates(
    ...
    if revision == 0 {
        return transaction.execute(
            "DELETE FROM octoscript_storage_versions WHERE namespace = ?1 AND name = ?2",
...
fn record_commitment(
    ...
    hasher.update(b"octoscript-storage/anchored-sqlite-record/v1");
...
mod tests {
    ...
    fn temporary_database_path(test_name: &str) -> PathBuf {
        ...
        let path = std::env::temp_dir().join(format!(
            "octoscript-storage-{test_name}-{}-{sequence}.sqlite",
            std::process::id()
        ));
    ...
    fn rejects_an_oversized_anchored_blob_before_materializing_it() {
        ...
        store
            .connection
            .execute(
                "UPDATE octoscript_storage_versions
    ...
    fn stale_writer_cannot_repopulate_candidates_after_recovery() {
        ...
        let candidates: i64 = store
            .connection
            .query_row(
                "SELECT COUNT(*) FROM octoscript_storage_versions WHERE namespace = ?1 AND name = ?2",
                params![key.namespace(), key.name()],
                |row| row.get(0),
            )
            .unwrap();
