...
fn main() {
    eprintln!("octoscript-cgroup-runner is supported only on Linux");
    std::process::exit(64);
}
...
fn main() {
    if let Err(error) = linux::run(std::env::args_os().skip(1)) {
        eprintln!("octoscript-cgroup-runner: {error}");
...
mod linux {
    ...
    mod tests {
        ...
        const CLOEXEC_HELPER_ENV: &str = "OCTOSCRIPT_CGROUP_RUNNER_CLOEXEC_HELPER";
        ...
        fn parses_a_fixed_bubblewrap_command_with_preserved_descriptors() {
            let configuration = RunnerConfiguration::parse(arguments(&[
                "--cgroup-procs",
                "/sys/fs/cgroup/octoscript-1/cgroup.procs",
                "--preserve-fd",
                "9",
                "--preserve-fd",
                "11",
            ...
            assert_eq!(
                configuration.cgroup_procs,
                std::path::Path::new("/sys/fs/cgroup/octoscript-1/cgroup.procs")
            );
