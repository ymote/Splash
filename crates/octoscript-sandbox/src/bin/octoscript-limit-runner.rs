...
fn main() {
    eprintln!("octoscript-limit-runner is supported only on Linux");
    std::process::exit(64);
}
...
fn main() {
    if let Err(error) = linux::run(std::env::args_os().skip(1)) {
        eprintln!("octoscript-limit-runner: {error}");
...
mod linux {
    ...
    mod tests {
        ...
        fn parses_each_supported_limit_before_a_fixed_command() {
            ...
            assert_eq!(configuration.command, "/opt/octoscript/worker");
        ...
        fn rejects_ambiguous_or_unbounded_configuration() {
            ...
            assert!(matches!(
                RunnerConfiguration::parse(arguments(&["--", "/opt/octoscript/worker"])),
