//! Linux cgroup-v2 resource policy for a fixed Octoscript worker launch.
//!
...
///
/// `parent` must name a host-owned, delegated cgroup. Octoscript does not enable
...
fn create_child_cgroup(parent: &Path) -> Result<PathBuf, CgroupV2PrepareError> {
    for _ in 0..MAX_CREATE_ATTEMPTS {
        ...
        let path = parent.join(format!("octoscript-{}-{sequence}", std::process::id()));
