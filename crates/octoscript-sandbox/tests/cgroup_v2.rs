...
use octoscript_sandbox::bubblewrap::{BubblewrapWorkerPolicy, ReadOnlyMount};
use octoscript_sandbox::cgroup_v2::{CgroupV2Limits, CgroupV2Policy};
...
impl TestCgroupParent {
    fn create() -> io::Result<Self> {
        ...
        let path = root.join(format!(
            "octoscript-cgroup-v2-integration-{}-{}",
            std::process::id(),
...
fn cgroup_policy(parent: &TestCgroupParent) -> CgroupV2Policy {
    CgroupV2Policy::new(
        &parent.path,
        env!("CARGO_BIN_EXE_octoscript-cgroup-runner"),
        cgroup_limits(),
    )
    .unwrap()
}
...
fn applies_limits_before_exec_and_kills_the_runner_subtree() {
    ...
    let mut worker = Command::new(env!("CARGO_BIN_EXE_octoscript-cgroup-runner"))
        .args([
            "--cgroup-procs",
