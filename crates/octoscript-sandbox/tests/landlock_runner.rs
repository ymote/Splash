...
fn landlock_runner() -> &'static str {
    env!("CARGO_BIN_EXE_octoscript-landlock-runner")
}

fn cgroup_runner() -> PathBuf {
    fs::canonicalize(env!("CARGO_BIN_EXE_octoscript-cgroup-runner")).unwrap()
}

fn limit_runner() -> PathBuf {
    fs::canonicalize(env!("CARGO_BIN_EXE_octoscript-limit-runner")).unwrap()
}
...
fn only_an_explicitly_allowed_program_can_be_executed() {
    ...
    // octoscript-limit-runner (and, when dynamically linked, its ELF loader) was
