# Octoscript Language Profile v0.2

This profile defines the portable, LLM-oriented subset exercised by the
standalone runtime. [Octoscript Grammar v0.2](grammar.md) is the normative source
producer contract. The `grammar_v0_2.octoscript` fixture is the normative
grammar-version regression, while `workflow_language.octoscript` exercises the
...
`octoscript_core::ui_l0::check_ui_l0` and realize them through the UI API.
`octoscript profile` describes this workflow profile; see
[Generated UI source](grammar.md#generated-ui-source) for the card entry point.

## Source contract

Provide normal Octoscript source. The runtime adds its own internal terminal
marker, so generated code must not depend on Makepad widget-host framing.
Before generating source, an LLM host may call `octoscript profile`. Its versioned
...
normative [Octoscript Grammar v0.2](grammar.md), a host tool catalog, a capability
...
For a multi-step proposal, call `octoscript workflow-schema` before producing a
workflow draft JSON document. It emits a JSON Schema draft plus an `x-octoscript`
...
operation handles. Submit the resulting document to `octoscript workflow-review`
...
Run `octoscript check <file>` before execution when an LLM or editor produced the
...
and tracked retained Octoscript VM storage to 8 MiB by default through
...
`octoscript format <file>` applies the same profile and compatibility checks, then
...
`octoscript format --check <file>` is the non-writing CI/editor form. Format before
requesting execution, then use `octoscript check` when a structured diagnostic
...
`octoscript_core::top_level_declarations` or its named, limit-aware variant. The
...
`octoscript outline <file>` exposes that result as structured JSON for local LLM
...
`octoscript_core::lexical_symbol_report` or its named, limit-aware variant. The
...
`octoscript_core::module_import_report` or its named, limit-aware variant. Each
...
guarded same-document rename. Octoscript validates the new name through
`octoscript_core::is_canonical_identifier`, reparses the bounded rewritten source,
...
`octoscript_core::lexical_completion_report` or its named, limit-aware variant.
...
bounded advisory projection through `initializationOptions.octoscript.toolCatalog`
or a later `settings.octoscript.toolCatalog` configuration update. That LSP-only
...
`octoscript_core::static_record_shape_report` or its named, limit-aware variant.
...
An editor may separately supply `initializationOptions.octoscript.moduleCatalog`
or a later `settings.octoscript.moduleCatalog` update: a bounded list of canonical
...
non-object `settings.octoscript` clears all advisory catalogs. See [Editor module
...
`initializationOptions.octoscript.workflowDataCatalog` projection. It contains only
...
hover. `octoscript-workflow` can derive this pair from a validated dataflow prefix,
...
`octoscript_core::tool_call_hint_report` or `tool_call_hint_report_named`; `octoscript
...
non-serializable runtime leases; it is not Octoscript-visible authority.
...
`review` path and the `octoscript workflow-review` CLI command return the same
...
tokens; Rust values enter Octoscript only through reviewed host adapters.
...
imports a frozen Octoscript-owned scalar module with constants `math.pi` and
`math.e`, plus:
...
`use mod.std.json` imports a frozen Octoscript-owned data module with
...
serialization accepts only JSON-safe Octoscript values. Both directions enforce
...
`use mod.std.text` imports a frozen Octoscript-owned module for local string data
...
`use mod.std.array` imports a frozen Octoscript-owned module for local collection
...
output items. Canonical Octoscript deliberately has no range operator. Transforms
...
`use mod.std.object` imports a frozen Octoscript-owned module for local record
...
host-side authority object, not a Octoscript value or a serialized credential.
...
serialize the supplied Octoscript record or array, while their results remain JSON
...
returns to Octoscript; metadata-only schemas in the catalog do not enforce input
...
data; the contract is not Octoscript-visible, serializable, or selected from a
...
or cancels it later. This does not expose an external API to Octoscript source;
...
For external tools, retry policy is also owned by the host. Octoscript source has
...
become a Octoscript value or a script-level stream; `await()` still resolves only
...
is entirely outside the language: Octoscript source cannot see the operation key,
...
Workflow restart state is also host-owned. Octoscript source cannot create or load
...
- Follow the [canonical grammar](grammar.md), run `octoscript format`, then use
  `octoscript check` before requesting execution.
- Do not use Makepad compatibility syntax such as `var`, `match`, the legacy
  ...
  members; `octoscript check` rejects it.
...
- Do not assume external progress output is readable from Octoscript source; use
  the terminal promise result supplied by the host.
- Keep effectful work in named tools and pure transformations in Octoscript code.
...
- Treat an approval as a bound set of names and call limits, including when a
  tool name comes from a computed string; it cannot be widened from Octoscript.
