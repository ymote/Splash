...
without exposing worker state, a journal, or a storage key to Octoscript source.
...
`octoscript-worker::WorkerSessionAdmission` must validate the authenticated session
...
accepted from Octoscript source or a worker request.
...
it. The included `octoscript-storage` memory backend remains development-only and
